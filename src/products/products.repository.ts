...
export class ProductsRepository {
    ...
    try {
      ...
      if (!Array.isArray(productDto)) {
        ...
        if (typeof category === 'string') {
          ...
          newProduct = queryRunner.manager.create(Products, {
            ...productDetails,
  ...
  async updateProduct(id:string, updateProductDto: UpdateProductDto) {
    ...
    try {
      ...
      if (category && Array.isArray(category)) { //CORREGIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIR
        //borra las que existen en la DB
        await queryRunner.manager.delete(Categories, { products: { id } })
        // mapeo las nuevas categories
        product.category = await category.map((category) => this.categoriesRepository.create({ name: category}))        
      }
