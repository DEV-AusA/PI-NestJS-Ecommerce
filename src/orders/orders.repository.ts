      ...
      if (isUUID(userId)) {
        ...
        if (!user)
          throw new NotFoundException(
            `No existe un usuario con el id ${userId}`,
          );
        ...
        const productsOrder = await Promise.all(
          products.map(async (product) => {
            ...
            if (!newProduct)
              throw new NotFoundException(
                `Es producto con id ${product.id} no existe.`,
              );
            return newProduct;
          }),
        );

        const availableProducts = productsOrder.filter(
        ...
        if (availableProducts.length !== productIds.length)
          throw new BadRequestException(
            `Algunos articulos estan fuera de stock.`,
          );
        // newOrderDetails #2
        ...
        const newOrderDetailsSaved =
          await queryRunner.manager.save(newOrderDetails);

        await queryRunner.commitTransaction();

        const lastOrder = await this.ordersRepository.findOneBy({
          id: savedOrder.id,
        });
        ...
        const result = {
          ...
          order_details: { id, price: +price.toFixed(2) },
        };
        return result;
      }
    } catch (error) {
