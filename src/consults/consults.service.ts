...
export class ConsultsService {
  constructor(private readonly embeddingCoseno: EmbeddingCosineService) {}
  ...
  async consultProduct(consultUser: ConsultDto) {
    ...
    try {
      ...
      const questionToEmbedding = await modelEmbedding.embedContent(
        consultUser.consult,
      );
      ...
      const embeddingsMatch =
        await this.embeddingCoseno.calculateEmbeddingCosineDistance(
          questionEmbedding,
          'descriptionEmbedding',
        );
      const embeddingsResult = embeddingsMatch.map((product) => ({
